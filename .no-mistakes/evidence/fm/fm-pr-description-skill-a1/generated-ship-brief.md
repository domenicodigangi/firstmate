...
1. First action: create your branch: `git checkout -b fm/prdesc-ship-a1 --`
...
4. Report status by appending one line:
   `echo "{state} [at=<epoch>]: {one short line}" >> '/tmp/fm-prdesc.L7Qepz/state/prdesc-ship-a1.status' && { [ ! -e '/tmp/fm-prdesc.L7Qepz/config/fleet-ledger' ] || '/home/ddg/.no-mistakes/worktrees/67bf63ea0b24/01M3FRE4F8RCJ4NZDD4PFDZV3M/bin/fm-fleet-ledger.sh' appended '/tmp/fm-prdesc.L7Qepz/config' '/tmp/fm-prdesc.L7Qepz/state/prdesc-ship-a1.status' >/dev/null 2>&1 || true; }`
...
6. If a decision belongs above the implementation worker (product choices, destructive actions),
   ...
   For a no-mistakes ask-user gate specifically, escalate all ask-user findings as one event plus one snapshot file, using that same shape even when the gate holds only a single ask-user finding: write only the ask-user findings, verbatim and unparaphrased (id, severity, file, line, description, authority), to `/tmp/fm-prdesc.L7Qepz/data/prdesc-ship-a1/nm-<run>-findings.txt`, then report the gate with
   `needs-decision [at=<epoch>] [key=nm-<run>-<step>]: ask-user findings=<id1>,<id2>,... file=/tmp/fm-prdesc.L7Qepz/data/prdesc-ship-a1/nm-<run>-findings.txt`
...
Firstmate steers you through durable message files in '/tmp/fm-prdesc.L7Qepz/state/prdesc-ship-a1.inbox'.
When a terminal message says an instruction is waiting there - and at any natural checkpoint when you are unsure - list '/tmp/fm-prdesc.L7Qepz/state/prdesc-ship-a1.inbox'/*.msg, read and act on each message in numeric order, then acknowledge each handled message by moving it: `mv '/tmp/fm-prdesc.L7Qepz/state/prdesc-ship-a1.inbox'/NNN.msg '/tmp/fm-prdesc.L7Qepz/state/prdesc-ship-a1.inbox'/handled/`.
...
A correction edits only the wrong text: do not run `/home/ddg/.no-mistakes/worktrees/67bf63ea0b24/01M3FRE4F8RCJ4NZDD4PFDZV3M/bin/fm-ensure-agents-md.sh`, create either file, or add sections, headings, or pointers alongside it.
